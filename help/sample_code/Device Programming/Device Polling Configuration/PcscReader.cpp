...
PcscReader::PcscReader()
{
	...
	PcscProvider_ = new PcscProvider();
}

PcscReader::PcscReader(PcscReader & rhs, string sReaderName)
{
	hContext_	= rhs.hContext_;
	sReaderName_ = sReaderName;
	pReaderStateList_ = NULL;
	uReaderStateListCount_ = 0;
}

PcscReader::PcscReader(string sReaderName)
{
	sReaderName_ = sReaderName;
	pReaderStateList_ = NULL;
	uReaderStateListCount_ = 0;
...
PcscReader::~PcscReader()
{
	...
	if(PcscProvider_ != NULL)
		delete PcscProvider_;
}
...
void PcscReader::initialize()
{
	...
	{
		errorMsg = PcscProvider_->getSCardErrorMsg(iResult);
		throw PcscException(iResult, errorMsg);
	}
}
...
void PcscReader::listReaders(StringList &rReaderList)
{
	...
	if (iResult != SCARD_S_SUCCESS)
	{
		char *errorMsg;
		errorMsg = PcscProvider_->getSCardErrorMsg(iResult);
		throw PcscException(errorMsg);
		//throw PcscException("List readers Fail");
	}	
...
int32 PcscReader::connect(string sReaderPort)
{
	...
	if (iResult != SCARD_S_SUCCESS)
	{
		...
		errorMsg = PcscProvider_->getSCardErrorMsg(iResult);
		throw PcscException(errorMsg);
		//throw PcscException("Connect Fail");
	}
	return 0;
}
...
{
	SCardDisconnect(hCardHandle_,SCARD_UNPOWER_CARD);
	return 0;
}
...
int32 PcscReader::getStatus(string sReaderName)
{
	DWORD rdrLen = 100;
	int32 iResult;
	DWORD dwState;

	iResult = SCardStatus(hCardHandle_, (char*)sReaderName.data(), &rdrLen, &dwState, &uActiveProtocol_, aAtr_, &uAtrLen_);
								  ...
								  uint32 &	rResponseLen)
{
	...
	{		
		...
		errorMsg = PcscProvider_->getSCardErrorMsg(iResult);
		throw PcscException(errorMsg);
		//throw PcscException("Transmit error");
	}
...
int32 PcscReader::sendControlCommand()
{
	uint32 uSendLen;
	uint32 uRecvLen;
	...
	uint8* pRecvBuffer = NULL;
	uint8* pSendBuffer = NULL;
    uint8 uApduCommandLength = 5;
    uint8 uStatusWordLength = 2;
	Apdu* pApdu;
	...
	if (pApdu->getSendData() != NULL)
        memcpy(pSendBuffer + uApduCommandLength, pApdu->getSendData(), uSendLen - uApduCommandLength);
	...
	if (uRecvLen != 0)
	{
		...
		memset(pRecvBuffer,0,uRecvLen);
	}	

	try
    {
		uOperationControlCode = 0x310000 + 3500 * 4;
		sendControlCommand(pSendBuffer,uSendLen,pRecvBuffer,uRecvLen,uOperationControlCode);
    }
	catch (PcscException cException)
	{
		if (pSendBuffer != NULL)
	...
	pApdu->setReceiveLen(uRecvLen);
	memcpy(pApdu->getReceiveData(),pRecvBuffer,uRecvLen);
    memcpy(pApdu->getStatusWord(), pRecvBuffer + (uRecvLen - uStatusWordLength), uStatusWordLength);
									 ...
									 DWORD uOperationControlCode)
{
	...
	iResult = SCardControl( hCardHandle_, uOperationControlCode,
							pCommand, uCommandLen,
	...
	rResponseLen = uActualResponseLen;
...
int32 PcscReader::connectDirect(CString readerName)
{
	int32 iResult;

	iResult = SCardConnect(hContext_, readerName, SCARD_SHARE_DIRECT, SCARD_PROTOCOL_UNDEFINED, &hCardHandle_, &uActiveProtocol_);
