...
UINT PcscReader::MainPollingThread( LPVOID pParam )
{
	...
	{	
		pReader->pPollingCallback_(POLL_STATUS_ERROR, NULL, 0,
	...
	for (uIdx = 0; uIdx < pPollingList->uNumReaders; uIdx++)
	{
		...
		{
			pReader->pPollingCallback_(POLL_STATUS_NOT_FOUND, NULL, 0,
						(void*)pReaderStates[uIdx].szReader);
		}
		if (ISBITSET(pReaderStates[uIdx].dwEventState,SCARD_STATE_PRESENT))
		{
			pReader->pPollingCallback_(POLL_STATUS_FOUND, pReaderStates[uIdx].rgbAtr, pReaderStates[uIdx].cbAtr,
						(void*)pReaderStates[uIdx].szReader);
		}
	}
		...
		for (uIdx = 0; uIdx < pPollingList->uNumReaders; uIdx++)
		{
			...
			{				
				...
				{	
					...
					{
						pReader->pPollingCallback_(POLL_STATUS_NEW_READER_FOUND, NULL, 0, NULL);
					}
					else
					{
						pReader->pPollingCallback_(POLL_STATUS_READER_REMOVED, NULL, 0, NULL);
					}					
				}
