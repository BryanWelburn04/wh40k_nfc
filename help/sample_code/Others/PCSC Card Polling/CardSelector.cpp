...
CardSelector::CardSelector(PcscReader *pPcscReader, string sReaderName)
{
...
CString CardSelector::readCardType(uint8 *pAtr, uint32 uAtrLen)
{
	...
	else // Part 4
	{
		...
		else
		{
			try
			{
				if(pReader_->connect(sReaderName_) == 0)
					sCardName = getUid(pUid, uUidLength);
			}
			catch(PcscException cExeption)
			{
				throw cExeption;
			}
		}
...
CString CardSelector::getUid(uint8 *pUid, uint32 &uUidLength)
{
	byte aResponse[100];
