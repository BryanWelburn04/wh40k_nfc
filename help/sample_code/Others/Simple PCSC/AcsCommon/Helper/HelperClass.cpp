...
void SecurityAttribute::securityAttribute(uint8 securityAttribute)
{
...
uint8 SecurityAttribute::getRawValue()
{
	uint8 rawValue = 0x00;
...
void InquireAccountResponse::setMac(uint8* mac)
{
	_mac = mac;
}

void InquireAccountResponse::setTransactionType(uint8 transactionType)
{
...
void InquireAccountResponse::setAtref(uint8* atref)
{
	_atref = atref;
}
...
void InquireAccountResponse::setTtrefc(uint8* ttrefc)
{
	_ttrefc = ttrefc;
}

void InquireAccountResponse::setTtrefd(uint8* ttrefd)
{
	_ttrefd = ttrefd;
}

uint8* InquireAccountResponse::inquireAccountResponse(uint8* rawDataFromCard)
{
	if(rawDataFromCard == NULL)
...
void OptionRegister::optionRegister(uint8 optionRegisterByte)
{
...
uint8 OptionRegister::getRawValue()
{
	uint8 rawValue = 0x00;
...
void SecurityOptionRegister::securityAttribute(uint8 securityAttribute)
{
	if ((securityAttribute & 0x01) == 0x80)
        _issuerCode = true;
...
uint8 SecurityOptionRegister::getRawValue()
{
	uint8 rawValue = 0x00;
