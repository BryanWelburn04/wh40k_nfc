...
void DES2 (BYTE *Data, BYTE *Key, int bDecrypt = 0)
{
	CipherKey = Key;
	bDecrypt ? Encrypt (Data) : Decrypt (Data);
}

void DES6 (BYTE *Data, BYTE *Key, int bDecrypt = 0)
{
	::CipherKey = Key;
	bDecrypt ? Encrypt (Data) : Decrypt (Data);
	::CipherKey = &Key[8];
	bDecrypt ? Decrypt (Data) : Encrypt (Data);
	::CipherKey = Key;
	bDecrypt ? Encrypt (Data) : Decrypt (Data);
}
