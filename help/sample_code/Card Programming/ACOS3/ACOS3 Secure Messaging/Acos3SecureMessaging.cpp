						  ...
						  int bDes)
{
	...
	if(SequenceNumber[7] == 255)
	{
		if(SequenceNumber[6] == 255)
		{
			SequenceNumber[6] = 0x00;
			SequenceNumber[7] = 0x00;
		}
		else
			SequenceNumber[6] = SequenceNumber[6] + 1;
	}
	else
		SequenceNumber[7] = SequenceNumber[7] + 1;
	...
	byte uStatusWord[2];
	char * errorMessage;
	uStatusWord[0] = apdu->getReceiveData()[2];
	uStatusWord[1] = apdu->getReceiveData()[3];

	if(uStatusWord[0] == 0x6A && uStatusWord[1] == 0x83)
	{
		errorMessage = getErrorMessage(uStatusWord);			
		
		delete apdu;
		delete [] response;
		throw CardException(errorMessage ,uStatusWord);
	}
	else if(uStatusWord[0] == 0x67 && uStatusWord[1] == 0x00)
	{
		errorMessage = getErrorMessage(uStatusWord);			
