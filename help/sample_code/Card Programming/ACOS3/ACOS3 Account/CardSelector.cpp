...
CardSelector::CardSelector(PcscReader *pPcscReader)
{
	pReader_ = pPcscReader;
}
...
CString CardSelector::readCardType(uint8 *pAtr, uint32 iAtrLen)
{
	byte aRid[] = { 0xA0, 0x00, 0x00, 0x03, 0x06 };
	byte aTempRid[5];	
	byte aCardIdentifier[2];
	uint8 *pUid = new byte[300];
	uint32 uUidLength;
	int i = 0;
	CString sCardName = "Unknown";

	// Check if Part 3 or Part 4
	if(pAtr[4] == 0x80 && pAtr[5] == 0x4F) // Part 3
	{
		// Check RID
		for(i = 0; i < 5; i++)
		{
		...
		if(memcmp(aTempRid, aRid, 5) == 0)
		{
			// Get Card Name
			for(i = 0; i < 2; i++)
			{
				aCardIdentifier[i] = pAtr[i + 13];
			}
	...
	else // Part 4
	{
		...
		else
		{
			...
			//Success = Type A
			//Fail = Type B
			sCardName = getUid(pUid, uUidLength);
		}
	}
...
CString CardSelector::getUid(uint8 *pUid, uint32 &rUidLength)
{
	
	byte aResponse[100];
	byte aCommand[5] = { 0xFF, 0xCA, 0x01, 0x00, 0x00 };	
	byte aStatusWord[2];
	...
	memcpy(aStatusWord,pApdu->getReceiveData() + pApdu->getReceiveLen() - 2, 2);
	if(aStatusWord[0] == 0x6A) // ISO 14443 Part 4 Type B
	{
		sCardType = "ISO 14443 Part 4 Type B";
	}
	else  // ISO 14443 Part 4 Type A; includes ACOS cards
	{
		sCardType = getCardVersion();
...
CString CardSelector::getCardVersion()
{
	byte *pCardVersion;
	Acos3* pAcos3 = new Acos3(pReader_);
	byte aVersion[5] = {0x41, 0x43, 0x4F, 0x53, 0x03};

	try
	{
		pCardVersion = pAcos3->getCardInfo(VERSION_NUMBER);
	}
	catch(PcscException cPcscException)
	{
		delete pAcos3;

		throw cPcscException;
	}
	catch(CardException cCardException)
	{
		delete pAcos3;

		if(cCardException.getStatusWord()[0] == 0x69 && cCardException.getStatusWord()[1] == 0x85)
			return "Unknown";
		else if(cCardException.getStatusWord()[0] == 0x6E && cCardException.getStatusWord()[1] == 0x00)
			return "Unknown";
		else if(strstr(cCardException.getMessage(), "Unknown Status Word") > 0)
			return "Unknown";
		else
			throw cCardException;
	}

	if(memcmp(aVersion, pCardVersion, sizeof(aVersion)) == 0)
		return "ACOS3";
	else
		return "Unknown";
}
