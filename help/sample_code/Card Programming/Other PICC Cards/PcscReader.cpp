...
void PcscReader::initialize()
{
	...
	if (iResult != SCARD_S_SUCCESS)
	{
		...
		//throw PcscException("Establish context Fail");
	}
}
...
void PcscReader::selectReader(string sReaderName)
{
...
int32 PcscReader::getAtr(uint8 *pAtr, uint32 &rAtrLen)
{
	...
	iResult = SCardStatus(hCardHandle_, aReader, &rdrLen, &dwState, &uActiveProtocol_, aAtr_, &uAtrLen_);
								  ...
								  uint32 &	rResponseLen)
{
	...
	setActualLength(uActualResponseLength);
									 ...
									 uint32 &rResponseLen)
{
	...
	if ( iResult != SCARD_S_SUCCESS )
	{
		...
		errorMsg = PcscProvider_->getSCardErrorMsg(iResult);
		throw PcscException(errorMsg);
		//throw PcscException("Scard Control Error");
	}
	return 0;
}
