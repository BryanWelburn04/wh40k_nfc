...
{	
	...
	{
		delete pApdu;
		throw CardException("Load Key failed");
	}
...
{	
    ...
    if (pReader_->getApduCommand()->getStatusWord()[0] != 0x90)
	{
		...
		byte aStatusWord[] = { pReader_->getApduCommand()->getStatusWord()[0],
							   pReader_->getApduCommand()->getStatusWord()[1]};
		delete pApdu;
		throw CardException("Authenticate failed", aStatusWord);
...
void MifareClassic::valueBlock(byte blockNumber, VALUEBLOCKOPERATION transType, int amount)
{
	...
	{
		delete apdu;
		throw CardException("Value Block Operation failed");
	}
...
int MifareClassic::inquireAmount(byte blockNumber)
{
	...
	byte command[5] = { 0xFF, 0xB1, 0x00, blockNumber, 0x00 };	
	...
	{
		delete apdu;
		throw CardException("Inquire Operation failed!");
	}
...
void MifareClassic::restoreAmount(byte sourceBlock, byte targetBlock)
{
	...
	{
		delete apdu;
		throw CardException("Restore Value failed");
	}
}
...
{	
	...
	{
		delete apdu;
		throw CardException("Read failed");
	}
...
void MifareClassic::updateBinary(byte blockNumber, byte* sendData, byte len)
{
	...
	{
		delete apdu;
		throw CardException("Update failed");
	}
	delete apdu;
}
